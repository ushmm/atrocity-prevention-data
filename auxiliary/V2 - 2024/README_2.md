...
- *code_guide_2.csv*: The ordered list of researchers responsible for coding each article, which we use to filter the excerpt-level data from Dedoose.

- *data_dictionary.csv*: A guide to the variables in the *all_factors.csv* and *all_sources.csv* datasets.

- *excerpts_2.csv*: The excerpt-level data from Dedoose, the qualitative coding software that we use for this project. This is the cumulative data from Dedoose, combining excerpts from the most recent studies and studies that we included in previous project updates.

- *master_codes_2.csv*: The list of codes that researchers applied to studies in Dedoose.
...
excerpts <- read.csv("excerpts_2.csv", na.strings = "", check.names = F) %>%
  dplyr::select(-317)
...
code_guide <- read.csv("code_guide_2.csv", na.strings = "")

### master list of codes
codes <- read.csv("master_codes_2.csv")
...
references <- read.csv("zotero_2.csv", encoding = "UTF-8") %>%
...
write.csv(references_index, "citations_2.csv", fileEncoding = "UTF-8")
...
work_dir_ag_2 <- "G:/.shortcut-targets-by-id/1gvhqlwU_FnML3ntwXRBg_r48BjM6CRsK/Lessons learned/Research evidence review/Analysis/2/Analytic guide"
setwd(work_dir_ag_2)

### rerun analytic guide code
source("G:/.shortcut-targets-by-id/1gvhqlwU_FnML3ntwXRBg_r48BjM6CRsK/Lessons learned/Research evidence review/Analysis/2/Analytic guide/analytic_guide_2.R")

### load analytic guide
analytic_guide <- read.csv("analytic_guide_2.csv")
...
work_dir_2_csv <- "G:\.shortcut-targets-by-id\1gvhqlwU_FnML3ntwXRBg_r48BjM6CRsK\Lessons Learned\Research evidence review\Analysis\V2"
setwd(work_dir_2_csv)
...
write.csv(factors, "all_factors_2.csv")

### full_article_level
write.csv(full_article_level, "all_sources_2.csv", fileEncoding = "UTF-8")

### CSV with outcomes information
write.csv(outcomes, "outcomes_2.csv", fileEncoding = "UTF-8")
