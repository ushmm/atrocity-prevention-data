...
work_dir <- "D:/Google Drive/CPG/Projects/Research/Lessons-Learned/Kyra's Folder/Lessons learned/1. Collecting, Distilling, and Organizing the Knowledge/Practitioner Knowledge/Survey/Analysis"
setwd(work_dir)
rm(work_dir)
...
practitioner <- practitioner %>%
  ...
  mutate(practitioner_index = seq(1:n()))
...
factors <- factors %>%
  mutate(Name = ifelse(factor == "atrocity_justifying_ideology" & direction == "Less", `Inverse factor`, Name),
         direction = ifelse(factor == "atrocity_justifying_ideology" & direction == "Less", "More", direction))

### invert display name for military_activity
factors <- factors %>%
  mutate(Name = ifelse(factor == "military_activity" & direction == "Less", `Inverse factor`, Name),
         direction = ifelse(factor == "military_activity" & direction == "Less", "More", direction))
...
### remove duplicate references to factors, calculate count of respondents who cite factor
factors <- factors %>%
  ungroup() %>%
  dplyr::select(-c(factor)) %>%
  distinct() %>%
  group_by(Name) %>%
  mutate(count_practitioner = n())
